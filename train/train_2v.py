...
Options:
  ...
  -c, --config=<config>   Config directory [default: config_2v].
...
def create_train_state( cfg: Any, data_shape: Tuple ) -> Tuple[nn.Module, nn.Module, Any, Any, Any, Any, Any, TrainState]:
  ...
  im_decode  = jnp.ones( (data_shape[0], 1, cfg.INPUT_SIZE[0], cfg.INPUT_SIZE[1], 3), dtype=dtype)
  
  pose_input = jnp.ones((data_shape[0], 2, 2, cfg.POSE_DIM), dtype=dtype)
...
def train_step(x: Any,  PoseEnc: nn.Module, Render: nn.Module, state: TrainState,
  ...
  key, pmask_key, aug_key, self_key = jax.random.split(key, 4)
  
  B, num_frames, H, W  = x.shape[0], x.shape[1], x.shape[2], x.shape[3]

  # Small chance that we decode reference frame
  self_mask = jax.random.uniform(self_key, shape=(B, )) <= 0.05

  x = x.at[:, -1, ...].set( (1.0 - self_mask)[:, None, None, None] * x[:, -1, ...] + self_mask[:, None, None, None] * x[:, 0, ...])
  
  # B x 2 x H x W x 3
  if pose_aug_map is not None and train:
    x_enc = pose_aug_map(aug_key, x)
  else:
    x_enc = x 
  ...
  def loss_fn(params):
    ...
    P = PoseEnc.apply({"params":params["PoseEnc"]}, x_enc, pmask)
    ...
    # B x H x W x 3   
    xtp_S = Render.apply({"params":params["Render"]}, x[:, 0, None, ...].astype(dtype), P, pmask)
    ...
    if not train:
      # B x H x W x 3
      xtp_T = Render.apply({"params":params["Render"]}, jnp.roll(x, 1, axis=0)[:, 0, None, ...].astype(dtype), P, pmask)
    ...
    if not train:
      ...
      xp_out = jnp.concatenate((x_viz[:, 0, None, ...], xtp_S[:, None, ...], jnp.roll(x_viz, 1, axis=0)[:, 0, None, ...], xtp_T[:, None, ...]), axis=1)
    else:
      x_out = x_viz
...
def train_and_evaluate( cfg: Any, output_dir: str ):
  ...
  exp_name     =  "xfactor_stereo_monocular_{}".format(cfg.EXP_ID)
  ...
  PoseEnc, Render, optimizer, w_dict, lpips, pose_aug_map, train_key, state = create_train_state( cfg, (4, *input_size, 3) )
  print( "Done..." )
  ...
  for step in tqdm( range(initial_step, stop_at) ):
    ...
    state, x_out = p_train_step( x=batch_in, state=state )
    ...
    if step % cfg.EVAL_EVERY == 0 or is_last_step:
      ...
      for j in range( cfg.NUM_EVAL_STEPS ):
        ...
        state, x_out = p_eval_step( x=batch_in, state=state )
