...
from ..heads.head_act import activate_head
from ..heads.utils import create_uv_grid, position_grid_to_embed


class VggtDptHead(nn.Module):
    """
    ...
    Args:
        ...
        dim_features (int, optional): Feature channels for intermediate representations. Default is 256.
    ...
    def __init__(
        ...
        self.scratch.refinenet1 = _make_fusion_block(dim_features)
        self.scratch.refinenet2 = _make_fusion_block(dim_features)
        self.scratch.refinenet3 = _make_fusion_block(dim_features)
        self.scratch.refinenet4 = _make_fusion_block(dim_features, has_residual=False)

        head_features_1 = dim_features
        head_features_2 = 32
    ...
    def forward(
        self,
        aggregated_tokens_list: List[torch.Tensor] | torch.Tensor,
        ...
        """
        H, W = image_size
        B,S,P,D = aggregated_tokens_list[0].shape

        # If frames_chunk_size is not specified or greater than S, process all frames at once
        if frames_chunk_size is None or frames_chunk_size >= S:
            return self._forward_impl(aggregated_tokens_list, image_size, patch_start_idx)
        ...
        for frames_start_idx in range(0, S, frames_chunk_size):
            ...
            if self.feature_only:
                chunk_output = self._forward_impl(
                    aggregated_tokens_list, image_size, patch_start_idx, frames_start_idx, frames_end_idx
                )
